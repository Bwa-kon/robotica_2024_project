...
from niryo_robot_led_ring.api import LedRingRosWrapper # type: ignore
from niryo_robot_sound.api import SoundRosWrapper # type: ignore

#import for actionserver
import actionlib
from my_niryo_robot_description.msg import ActionMsgAction, ActionMsgFeedback, ActionMsgResult # type: ignore
from my_niryo_robot_description.msg import scan_poseAction, scan_poseFeedback, scan_poseResult # type: ignore

##Define Pickup pose for the robot
# [x,y,z,rx,ry,rz]/[link1,link2,link3,link4,link5,link6]
pickup_pose = [0,0,0,0,0,0]

## this class is used as a structure so we can access the min and max value of a variable
class range:
    min = 0
    max = 0
...
##Go towards scan pose
    def __init__(self,pickup_pose):
        rospy.loginfo("Starting pick and place program, going to pre pick location.")
##initialising and bootup process
    ##initialize controller objects for the niryo
    ...
    ##Register attached tool
    self.niryo_robot.updated_tool()
    ...
    #get box size from param server
    #self.
    ...
    def scanpose_callback(self, goal):
        ...
        #pickup_pose=[1.46,0.22,-0.05,-0.0,-1.75,0]
...
if __name__ =='__main__':
    ...
    niryo = Pick_and_place(pickup_pose)
